...
const AI_PROMPTS = {
    rewrite: {
        description: 'You are a professional OFTEC oil heating engineer writing service-report observations for a UK oil heating safety company. Rewrite the following description of a fault on an oil-fired appliance, oil tank, supply pipework, flue or AGA/Rayburn/Stanley range so it is clear, professional and technically accurate. Reference relevant standards where appropriate (OFTEC TI/133, OFTEC TI/134, BS 5410, Building Regulations Part J). 2-3 sentences. Use British English. Do not add any preamble or explanation. Return only the rewritten text.',
        fix: 'You are a professional OFTEC oil heating engineer writing service-report observations for a UK oil heating safety company. Rewrite the following description of how an oil heating issue was resolved (e.g. burner strip-down, nozzle/oil filter change, combustion re-set, flue repair, tank rectification, supply-line work). Use past tense, action-focused language. 1-2 sentences. Use British English. Do not add any preamble. Return only the rewritten text.',
        outcome: 'You are a professional OFTEC oil heating engineer writing service-report observations for a UK oil heating safety company. Rewrite the following sentence summarising the final result of an oil heating repair or service. Keep it to 1 short sentence focused on the outcome (efficiency, compliance with OFTEC TI/133 or BS 5410, customer impact). Use British English. Do not add any preamble. Return only the rewritten text.'
    },
    proofread: {
        description: 'You are a copy editor for a UK oil heating safety company. Fix only spelling and grammatical errors in the following text. Do not rephrase, restructure, change the meaning, or add anything. Use British English. Return only the corrected text with no preamble.',
        fix: 'You are a copy editor for a UK oil heating safety company. Fix only spelling and grammatical errors in the following text. Do not rephrase, restructure, change the meaning, or add anything. Use British English. Return only the corrected text with no preamble.',
        outcome: 'You are a copy editor for a UK oil heating safety company. Fix only spelling and grammatical errors in the following text. Do not rephrase, restructure, change the meaning, or add anything. Use British English. Return only the corrected text with no preamble.'
    }
};
...
function callClaudeAPI(text, fieldType, mode) {
    return new Promise((resolve, reject) => {
        ...
        if (!apiKey) return reject(new Error('AI API key not configured. An admin must set it in /admin/ → Settings → AI Settings.'));
...
async function createCalendarEvent(lead) {
    ...
    try {
        ...
        const event = {
            summary: `Oil heating service — ${lead.fullName}`,
            description: [
...
http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && url.match(/^\/api\/leads\/([^/]+)\/send-review$/)) {
        ...
        try {
            ...
            const whatsappMessage = encodeURIComponent(
                `Hi ${customerName}, your CD/10 oil safety certificate is ready and has been sent to ${lead.email || 'your email'}. ` +
    ...
    if (url === '/admin' || url === '/admin/' || url === '/installer' || url === '/installer/') {
        const target = (url.startsWith('/admin') ? '/admin/' : '/installer') + '?from=oilcert';
    ...
    console.log(`OilCert server running at http://localhost:${PORT}`);
